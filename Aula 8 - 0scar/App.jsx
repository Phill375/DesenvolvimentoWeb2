...
function App() {
  ...
  const gerarEstiloBotao = (botao) => {
      if(botao === 'Ganhadores'){
        return{
         backgroundColor: oscar ? 'green' : 'gray',
         color: oscar ? 'white' : 'black',
        };
      }
      if( botao === 'Perdedores'){
      return{
        backgroundColor: oscar ? 'gray' : 'green',
  ...
  return (
   ...
   <h1>Oscar</h1>
   <div>
    <button onClick={handleOnClickGanhador} style ={gerarEstiloBotao('Ganhadores')}>Ganhadores</button>
    <button onClick={handleOnClickPerdedor} style ={gerarEstiloBotao('Perdedores')}>Perdedores</button>
   </div>
   {
